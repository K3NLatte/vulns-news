...
test('comments stay text; deletion and profile dialog return keyboard focus', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /自分のコメントを削除/ }).focus()
  await page.keyboard.press('Enter')
  await expect(input).toBeFocused()
  expect(await page.evaluate(() => (window as unknown as Record<string, unknown>).__injected)).toBeUndefined()
  const account = page.getByRole('navigation', { name: 'メインメニュー' }).getByRole('button', { name, exact: true })
  await account.click()
...
test('saved submitted report survives a new session, unsave/resave, and stays out of guest state', async ({ page, browser }) => {
  ...
  for (let round = 0; round < 2; round++) {
    ...
    try {
      ...
      await fresh.getByRole('link', { name: 'CVE-2026-12345をページで開く' }).click()
      await expect(fresh.getByRole('heading', { level: 1 })).toContainText('CVE-2026-12345')
      await fresh.getByRole('button', { name: '記事の保存を解除', exact: true }).click()
      await expect(fresh.getByRole('heading', { level: 1 })).toContainText('CVE-2026-12345')
      await fresh.getByRole('button', { name: '記事を保存', exact: true }).click()
...
test('analysis rejects unsafe input, deduplicates active requests and preserves cancellation on reload', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'CVE-2026-98765の解析を中止' }).click()
...
test('storage denial keeps the current session usable and reports unsaved changes', async ({ page }) => {
  ...
  await ready(page)
  await page.getByRole('button', { name: 'DEMO-2026-001を保存', exact: true }).click()
  await expect(page.getByRole('button', { name: 'DEMO-2026-001の保存を解除', exact: true })).toBeVisible()
  await expect(page.getByRole('alert').filter({ hasText: '保存できません' })).toBeVisible()
})
...
test('long accepted identifiers keep the detail body reachable', async ({ page }) => {
  await page.goto('/#/analyze')
  await submit(page, 'CVE-2026-' + '7'.repeat(600))
  await expect(page.locator('.request-entry').first().locator('.state-completed')).toBeVisible()
  await page.locator('.request-entry').first().getByRole('link').click()
  await page.getByRole('button', { name: '記事を保存', exact: true }).click()
  await page.goto('/#/saved')
  await expect(page.locator('.is-detail-scroll')).toBeVisible()
  const detail = page.locator('.feed-detail')
...
test('a stale profile tab cannot silently overwrite a newer saved list', async ({ page, context }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'DEMO-2026-001を保存', exact: true }).click()
    await second.getByRole('button', { name: 'DEMO-2026-002を保存', exact: true }).click()
    await expect(second.getByRole('alert')).toContainText('別のタブ')
    await page.reload()
    await expect(page.getByRole('button', { name: 'DEMO-2026-001の保存を解除', exact: true })).toBeVisible()
    await expect(page.getByRole('button', { name: 'DEMO-2026-002を保存', exact: true })).toBeVisible()
