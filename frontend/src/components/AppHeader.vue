...
defineProps<{ user: WorkspaceUser | null; fullFeatures: boolean; page: string; savedCount: number }>()
defineEmits<{ account: [] }>()
</script>
  ...
  <header class="app-header">
    ...
    <nav v-if="fullFeatures" class="header-actions" aria-label="メインメニュー">
      ...
      <a href="#/saved" :aria-current="page === 'saved' ? 'page' : undefined">
        <Bookmark :size="19" aria-hidden="true" /><span>保存済み</span><span v-if="savedCount" class="nav-count">{{ savedCount }}</span>
      </a>
      <a href="#/settings" :aria-current="page === 'settings' ? 'page' : undefined">
        <Settings :size="19" aria-hidden="true" /><span>設定</span>
      </a>
      <button type="button" @click="$emit('account')">
