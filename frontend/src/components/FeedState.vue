...
defineProps<{ state: 'loading' | 'error' | 'empty' | 'repository' | 'saved'; message?: string; hasFilters?: boolean }>()
...
<template>
  ...
  <div v-else class="empty-state" :role="state === 'error' ? 'alert' : 'status'">
    ...
    <button v-else-if="state === 'repository'" type="button" class="secondary-button" @click="$emit('repositories')">
      リポジトリを追加
    </button>
    <button
      v-else-if="state === 'empty' && hasFilters !== false"
      type="button"
