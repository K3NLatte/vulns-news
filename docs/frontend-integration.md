...
取得の合成は `services/localFeedLoader.ts` に閉じ込めています。実API接続時はこのアダプターを丸ごと置き換え、画面側で検索結果を再絞り込みしません。`FeedResult` は `nextCursor` とサーバー側の総件数を受け取り、追加ページをIDで重複排除します。不正な記事は件数を知らせて除外し、応答全体の形式不正は再試行可能なエラーにします。公開日・CVSS・製品・分析根拠などは欠損を表現できます。

`useFeed` は15秒のタイムアウト、中止、古い応答の破棄、更新時の既存結果保持を担当します。`RequestError` の公開メッセージへ変換し、HTTPエラー本文や例外文を直接表示しません。現在の待機時間はブラウザー用の初期値で、LLM処理自体の完了期限ではありません。

保存操作は `useWorkspace` の非同期Result、解析操作は `useReportLibrary.commands` の非同期Resultを画面へ返します。解析受付の `ReportCommandAdapter` は入力検証後に呼び出し、受付成功までローカル状態を変更しません。保留・拒否・競合・プロフィール切替を扱います。現在はブラウザー内の受付アダプターであり、実APIが採番したジョブの受信・認証は接続時に実装します。
