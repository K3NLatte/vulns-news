...
通常表示はGoのモックAPIに接続します。先に別ターミナルでリポジトリ直下から `go run ./server` を実行してください（現在のローカル起動用 `server/main.go` を使用）。一般フィード、リポジトリ向け表示、検索、重要度フィルター、並び替え、記事詳細を確認できます。記事・依存関係・AI分析はすべて架空サンプルで、実際の解析には接続していません。
